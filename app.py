...
# Top category straight from the data — no overrides
...
target_p = round(bo_cat["proteins_100g"].median()) if len(bo_cat) > 0 else PROTEIN_THRESHOLD
target_s = round(bo_cat["sugars_100g"].median())   if len(bo_cat) > 0 else SUGAR_THRESHOLD

# ── Sidebar ────────────────────────────────────────────────
with st.sidebar:
    st.markdown("""
    <div style="margin-bottom:1.8rem;padding-bottom:1.2rem;border-bottom:1px solid #DDD9CF;">
      <div style="font-family:'Playfair Display',serif;font-size:1.25rem;color:#1C1B18;font-weight:600;">Sugar Trap</div>
    ...
    selected_cats = st.multiselect(
        "Categories", options=all_cats, default=all_cats, key="sel_cats")

    st.markdown("<div style='margin:1rem 0;border-top:1px solid #EFECE5;'></div>", unsafe_allow_html=True)

    sugar_max   = st.slider("Max Sugar  g/100g",   0, 100, 100, 5,   key="sugar_max")
    protein_min = st.slider("Min Protein  g/100g", 0,  50,   0, 2,   key="protein_min")

    st.markdown("<div style='margin:1rem 0;border-top:1px solid #EFECE5;'></div>", unsafe_allow_html=True)

    sample_size = st.slider("Sample size / category", 100, 800, 400, 100, key="sample_size")
...
if not selected_cats:
    ...
    <div style="padding:4rem 3rem;text-align:center;">
      ...
      <div style="font-size:0.9rem;color:#8A8680;">Use the sidebar to select at least one category.</div>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
if not df_plot.empty:
    for cat, color in CATEGORY_COLORS.items():
        ...
        fig.add_trace(go.Scatter(
            ...
            customdata=sub[["product_name","sugars_100g","proteins_100g",
                            "fat_100g","fiber_100g"]].values.tolist(),
            hovertemplate=(
                "<b>%{customdata[0]}</b><br>"
                "Sugar: %{customdata[1]:.1f}g<br>"
                "Protein: %{customdata[2]:.1f}g<br>"
                "Fat: %{customdata[3]:.1f}g<br>"
                "Fiber: %{customdata[4]:.1f}g<extra></extra>"
            ),
        ))
...
fig.add_shape(type="rect", x0=0, x1=SUGAR_THRESHOLD, y0=PROTEIN_THRESHOLD, y1=70,
              ...
              line=dict(color="#B8860B", width=0.5, dash="dot"), layer="below")
fig.add_annotation(x=7.5, y=56, text="<b>BLUE OCEAN</b><br>High Protein · Low Sugar",
                   showarrow=False,
...
fig.update_layout(
    xaxis=dict(range=[-1,65], gridcolor=GRID_COLOR, zerolinecolor=GRID_COLOR,
               title="Sugar (g/100g)",
               title_font=dict(color=FONT_COLOR, size=11), tickfont=dict(color=FONT_COLOR, size=10)),
    ...
    plot_bgcolor=PLOT_BG, paper_bgcolor=PAPER_BG, height=540,
)
...
# ── Key Finding — driven by actual data ────────────────────
st.markdown(f"""
<div style="margin:0.5rem 3rem 0 3rem;background:#fff;border:1px solid #DDD9CF;
  ...
  <div style="font-size:0.98rem;line-height:1.8;color:#4A4844;">
    ...
    <strong style="color:#1C1B18;">~{target_p}g protein</strong> and
    <strong style="color:#1C1B18;">under {target_s}g sugar</strong> per 100g.
...
fig_l.add_trace(go.Scatter(
    x=scores_s, y=cats_s, mode="markers+text",
    marker=dict(size=13,
    ...
    textfont=dict(size=11, color=["#B8860B" if c == top_cat else "#8A8680" for c in cats_s],
                  family="IBM Plex Mono"),
    hovertemplate="%{y}: %{x:.3f}<extra></extra>",
))
fig_l.add_annotation(x=scores_s[0]+0.015, y=0, text="#1", showarrow=False,
                      font=dict(size=10, color="#B8860B", family="IBM Plex Mono"), xanchor="left")
fig_l.update_layout(
    xaxis=dict(title="Opportunity Score", range=[0,1.28], showgrid=True,
               gridcolor=GRID_COLOR, zeroline=False,
               title_font=dict(color=FONT_COLOR, size=10), tickfont=dict(color=FONT_COLOR, size=10)),
    yaxis=dict(autorange="reversed", tickfont=dict(size=11, color=TEXT_COLOR), showgrid=False),
    showlegend=False,
    margin=dict(l=10, r=110, t=10, b=40),
    plot_bgcolor=PLOT_BG, paper_bgcolor=PAPER_BG, height=340,
)
...
with me_col:
    ...
    <div style="background:#fff;border:1px solid #DDD9CF;padding:1.3rem 1.4rem;margin-top:0.5rem;">
      ...
      <div style="display:flex;justify-content:space-between;padding:0.5rem 0;
                  border-bottom:1px solid #EFECE5;font-size:0.88rem;">
...
fig_b = go.Figure(go.Bar(
    ...
    text=[f"{v:,}" for v in ingr_vals], textposition="outside",
...
fig_b.update_layout(
    ...
    plot_bgcolor=PLOT_BG, paper_bgcolor=PAPER_BG, height=320, bargap=0.4,
)
